...
class PolicyGradientREINFORCE(object):
  ...
  def sampleAction(self, states):
    ...
    if random.random() < self.exploration:
      ...
      action_scores = self.session.run(self.action_scores, {self.states: states.flatten()})[0]
  ...
  def updateModel(self):
    ...
    for t in xrange(N-1):
      ...
      _, summary_str = self.session.run([
        ...
        self.states:             states.flatten(),
        self.taken_actions:      actions.flatten(),
        self.discounted_rewards: rewards.flatten()
      })
